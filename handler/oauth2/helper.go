...
func (h *HandleHelper) IssueAccessToken(ctx context.Context, requester fosite.AccessRequester, responder fosite.AccessResponder, expiryClientSeconds int32) error {
	...
	responder.SetExpiresIn(getExpiresInByClient(requester, fosite.AccessToken, h.AccessTokenLifespan, time.Now().UTC(), expiryClientSeconds))
